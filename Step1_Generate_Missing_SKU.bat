...
echo Step 1 started %date% %time% >> logs\console.log
...
python "2-6-2026_Final_Working.py" --mode generate-missing >> logs\console.log 2>&1
set EXIT_CODE=%errorlevel%

if not "%EXIT_CODE%"=="0" (
    echo Step 1 failed. Check logs\console.log and the latest logs\run_*.log
) else (
    echo Step 1 finished.
    echo If Missing SKU.xlsx was created or marked incomplete, fill it and then run Step2_Run_Full_Pipeline.bat.
)

echo Step 1 finished %date% %time% with exit code %EXIT_CODE% >> logs\console.log
pause
exit /b %EXIT_CODE%
